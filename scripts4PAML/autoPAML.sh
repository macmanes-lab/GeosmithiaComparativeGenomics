...
for inputaln in $(ls *aln); do
    ...
    then
        ...
        raxmlHPC-PTHREADS-AVX -T 2 -m GTRGAMMA -n $F.tree -s $F.aln -p 12345 &&
        python ~/myscripts/testing_PAML/cds4paml/autoPAML_nullbranchsite.py $F.aln RAxML_bestTree.$F.tree $F.out &
    else
